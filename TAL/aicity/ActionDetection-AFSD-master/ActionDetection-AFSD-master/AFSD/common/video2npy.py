import sys
sys.path.append('E:\program\ActionDetection-AFSD-master\ActionDetection-AFSD-master')
import numpy as np
import cv2
import os
import pandas as pd
...
def video2npy(data_path, anno_path, save_path, sample_fps=10.0, resolution=112,
    ...
    for video_name in sorted(list(set(df['video'].values[:]))):
        ...
        print(os.path.join(data_path, video_name + '.mp4'))
        if not capture.isOpened():
        ...
        video_infos.append([video_name, fps, sample_fps, cur_count, save_count])
        # save to npy file
